...
const fetch_air_quality = async (latitude, longitude) => {
  const api = `${OPEN_WEATHER_MAP_BASE_URL}/data/2.5/air_pollution?lat=${latitude}&lon=${longitude}&appid=${OPEN_WEATHER_MAP_API_KEY}`;
...
const fetch_hourly_air_quality = async (latitude, longitude) => {
  const api = `${OPEN_WEATHER_MAP_BASE_URL}/data/2.5/air_pollution/forecast?lat=${latitude}&lon=${longitude}&appid=${OPEN_WEATHER_MAP_API_KEY}`;
  const fetch_air_quality = await fetch(api);
  const air_quality = fetch_air_quality.json();
